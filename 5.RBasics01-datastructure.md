...
其基础数据类型包括向量（vector）、矩阵（matrix）、数组（array）、数据框（data frame）和列表（list），
也可包括由这些实体定义的更一般性的结构（structures）。
实体、对象和结构，以及后面会涉及的“类”、“S3”、“S4”等，都是抽象的计算机术语，初学者不必过分在意其“本质定义”，仅从功能性或操作性的视角加以理解即可
^[同时，这里未从计算机程序语言的角度严格区分数据结构（data stucture）和数据类型（data type）这两个概念。
从已有文献看，R 的开发者也未对两者之间的定义做出明确区分，并多用“数据结构”一词指代一般计算机程序中的“数据类型”，
用于说明 R 中数据的储存形态、提取方式及相关的指定操作。因此，在本书中暂可将两者等同看待。]。
